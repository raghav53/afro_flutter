...
import 'package:afro/Model/Events/EventsUsers/EventsUsersDataModel.dart';
...
class EventUsesModel {
  bool? success;
  int? code;
  String? message;
  List<EventDataUser>? data;
  Metadata? metadata;

  EventUsesModel(
      {this.success, this.code, this.message, this.data, this.metadata});

  EventUsesModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json['data'] != null) {
      data = <EventDataUser>[];
      json['data'].forEach((v) {
        data!.add(new EventDataUser.fromJson(v));
      });
    }
